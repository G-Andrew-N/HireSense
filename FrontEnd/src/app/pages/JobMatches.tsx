...
import { mockJobMatches } from "../data/mockData";
...
export function JobMatches() {
  ...
  const filteredJobs = mockJobMatches.filter((job) => {
                    ...
                    <div className="flex-1 w-full sm:w-auto space-y-3 sm:space-y-4">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6 text-sm">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-gray-700 dark:text-gray-300">Posted {new Date(job.postedDate).toLocaleDateString()}</span>
                        </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {job.skills.map((skill) => (
                            <Badge key={skill} variant="outline" className="bg-green-50 dark:bg-green-950 text-green-700 dark:text-green-300 border-green-200 dark:border-green-800">
                              {skill}
                            </Badge>
                          ))}
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {job.missingSkills.map((skill) => (
                              <Badge key={skill} variant="outline" className="bg-orange-50 dark:bg-orange-950 text-orange-700 dark:text-orange-300 border-orange-200 dark:border-orange-800">
                                {skill}
                              </Badge>
                            ))}
                          </div>
                      ...
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 pt-2">
                        <Button className="bg-emerald-600 hover:bg-emerald-700 justify-center">
                          Apply Now
                          <ExternalLink className="w-4 h-4 ml-2" />
                        </Button>
                        <Button variant="outline" className="justify-center">Save for Later</Button>
                        <Button variant="ghost" className="justify-center">View Details</Button>
                      </div>
                    </div>
                  </div>
                </CardContent>
        ...
        {filteredJobs.length === 0 && (
          ...
          >
            <Card className="p-12 text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400">Try adjusting your filters or upload a new resume</p>
            </Card>
          </motion.div>
        )}
      </div>
