...
export function Insights() {
  ...
  // Check localStorage to see if user has ever auto-generated insights
  const hasAutoGeneratedRef = useRef(
    (() => {
      try {
        return localStorage.getItem("hiresense:has-auto-generated-insights") === "true";
      } catch {
        return false;
      }
    })()
  ...
  useEffect(() => {
    const autoGenerate = async () => {
      console.log('[Insights] Auto-generate check:', { loading, generating, hasAutoGenerated: hasAutoGeneratedRef.current, insightsCount: insights.length });
      
      // Only run after initial load completes
      if (loading) return;
      if (generating) return;
      if (hasAutoGeneratedRef.current) return; // Already auto-generated in this browser
      if (insights.length > 0) return; // Already has insights
      
      try {
        console.log('[Insights] Checking for resumes...');
        const resumes = await getResumes();
        const hasResume = Array.isArray(resumes) && resumes.length > 0;
        console.log('[Insights] Has resume:', hasResume, 'Resume count:', resumes?.length);
        
        if (hasResume) {
          console.log('[Insights] Triggering auto-generation');
          hasAutoGeneratedRef.current = true;
          try {
            localStorage.setItem("hiresense:has-auto-generated-insights", "true");
          } catch {}
          toast.info("Generating your first insights...");
          handleGenerate();
        }
      } catch (err) {
        console.error('[Insights] Auto-generate error:', err);
